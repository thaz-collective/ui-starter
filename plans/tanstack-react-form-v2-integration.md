...
export const { useAppForm } = createFormHook({
  fieldComponents: {
    ...
    // DateField, TimeField, NumberField, ... follow the same shape
  },
  formComponents: {},
})
```
...
- **date-field** (`src/common/components/date-field/`): RAC's `DateField` is generic over `T extends DateValue` (from `@internationalized/date` — `CalendarDate`/`CalendarDateTime`/`ZonedDateTime`) per `date-field.tsx`'s `DateFieldProps<T extends DateValue>`. This is the one component where a real value-shape adapter is needed if the form's canonical stored value is a plain string/ISO date/JS `Date` rather than a `CalendarDate` — the adapter's `onChange` must convert `DateValue -> form value` and its `value` prop must convert `form value -> DateValue` (or `null` when unset). Recommend deciding a canonical "form value" representation for dates project-wide (e.g. keep `CalendarDate` as the canonical value type end-to-end, avoiding conversion entirely, if consuming apps don't need a plain string) before designing this adapter further — this is a real design decision, not just an integration detail.
- **time-field** (`src/common/components/time-field/`): Same `DateValue`-family concern as date-field but for `Time`/`CalendarDateTime` — likely needs the same conversion strategy decided alongside date-field so the two stay consistent.
