...
3. `context.ts` + `-context-provider.tsx`: same shape as `checkbox`'s group context (`{ slots }`) so `Radio`, `Label`, `Description`, `FieldError` can react to group-level `data-invalid`/`data-disabled`.
